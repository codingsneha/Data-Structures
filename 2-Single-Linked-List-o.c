...
/* 2. INSERTION
   ...
   2.3. Inserion at any other position
*/
...
void insertEnd(int key, int data) {
   ...
   struct node *link = head;      //ptr is temporary variable initialized with head moves to last node
	
   while(link->next){
      link=link->next;             //ptr points to last node or stores the link to last node
   }
